...
outLasso <- cv.glmnet(X[trainFlag,], y[trainFlag], family="binomial", alpha=1)

  pred <- 1 / (1 + exp(-1 * cbind(1,X[,]) %*% coef(outLasso)))
  print(auc(roc(y[!trainFlag], pred[!trainFlag]))) # new



for (j in 2:length(outLasso$lambda)) {
  index <- which(coef(outLasso)[-1,j] != 0)
  out <- glm(y[trainFlag] ~ X[trainFlag,index], family="binomial")
  pred <- 1 / (1 + exp(-1 * cbind(1,X[,index]) %*% coef(out)))
  print(auc(roc(y[!trainFlag], pred[!trainFlag]))) # new
}
...
id <- which.max(X[,11])
system(paste0("open columbiaImages/",pm$name[id]))
