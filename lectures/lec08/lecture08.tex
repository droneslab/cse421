...
The hypothesis set $H_0: \beta_j = b_j$, yields the following
{\bf t-test}:
\begin{align*}
...
<<<<<<< HEAD
The hypothesis set $H_0: D\beta = d$, for a full rank $k$-by-$p$ matrix
yields the following {\bf F-test}
\begin{align*}
F &= \frac{(\text{SSR}_U -  \text{SSR}_R) / k }{\text{SSR}_U / (n - p)}\\
&\sim F_{k, n-p}
\end{align*}
Where $\text{SSR}_U$ is $r^t r$ and $\text{SSR}_R$ is the
sum of squared residuals $\tilde{r}^t \tilde{r}$ from the
restricted model.
